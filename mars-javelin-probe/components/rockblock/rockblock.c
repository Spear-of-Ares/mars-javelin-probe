...
*     Last Modified       :     [2022-10-04 02:32]
*     Description         :      
**********************************************************************************/
#include <stdio.h>
...
*/
static uint8_t rockblock_uart_xfer(rockblock_t *rb_modem, char *cmd, uint8_t **resp, rockblock_uart_response_et response_type)
{
  resp = NULL;
  ...
  strncpy(msg, AT, strlen(AT));
  strncpy(msg, cmd, strlen(cmd));
  strncpy(msg, AT_COMPLEMENT, strlen(AT_COMPLEMENT));
  ...
  if( uart_tx_chars(uart_num, msg, msg_len) && (response_type == RETURN_RESPONSE))
  {
    ...
    uint8_t *uart_response = calloc(uart_response_length);
    ...
    int result = uart_read_bytes(uart_num, uear_response, uart_response_length, TICKS_TO_WAIT);
    ...
    // Split into lines
    uint8_t resp_len = rockblock_uart_response_to_lines(uart_response, uart_response_length, resp);
  }
...
/**
  ...
  * @param rb_modem Adafruit RockBLOCK modem.
*/
static void rockblock_transfer_buffer(rockblock_t *rb_modem)
{
...
rockblock_st* rockblock_driver_install(uart_port_t uart_num, uint32_t baudrate)
{
  ...
  return {.uart_num = uart_num, .buf_out = uint8_t[BUF_OUT_SIZE], .buf_in = uint8_t[BUF_IN_SIZE]};
}
