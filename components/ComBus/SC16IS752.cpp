...
int16_t SC16IS752::SetBaudrate(uint8_t channel, uint32_t baudrate) // return error of baudrate parts per thousand
{
  ...
  SetPrescalar(channel, SC16IS752_PRESCALAR_1);
...
void SC16IS752::SetPrescalar(uint8_t channel, uint8_t prescalar_value)
{
  uint8_t temp_reg = ReadRegister(channel, SC16IS750_REG_EFR);
  WriteRegister(channel, SC16IS750_REG_EFR, temp_reg | 0x10); // set ERF[4] to '1' to use the  enhanced features

  uint8_t mcr_reg = ReadRegister(channel, SC16IS750_REG_MCR);
  if (prescalar_value == 0)
  {
    mcr_reg = mcr_reg & (0b01111111); // Clear MCR[7] bit
  }
  else
  {
    mcr_reg = mcr_reg | 0x80; // Set MCR[7] bit
  }
  WriteRegister(channel, SC16IS750_REG_MCR, mcr_reg); // Set MCR[7] to prescalar value to determine which prescalar to use
