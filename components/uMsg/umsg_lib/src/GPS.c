...
// msg instances
static umsg_msg_metadata_t msg_GPS_configuration = {.name = "GPS_configuration"};
static umsg_msg_metadata_t msg_GPS_data = {.name = "GPS_data"};
...
{
    return umsg_subscribe(&msg_GPS_configuration, prescaler, sizeof(umsg_GPS_configuration_t), length, 0);
}
umsg_sub_handle_t umsg_GPS_configuration_subscribe_ch(uint32_t prescaler, uint8_t length, uint8_t channel)
{
    return umsg_subscribe(&msg_GPS_configuration, prescaler, sizeof(umsg_GPS_configuration_t), length, channel);
}
void umsg_GPS_configuration_publish(umsg_GPS_configuration_t* data)
{
    umsg_publish(&msg_GPS_configuration, data, 0);
}
void umsg_GPS_configuration_publish_ch(umsg_GPS_configuration_t* data, uint8_t channel)
{
    umsg_publish(&msg_GPS_configuration, data, channel);
}
...
{
    return umsg_peek(&msg_GPS_configuration, data, sizeof(umsg_GPS_configuration_t));
}

// GPS_data
umsg_sub_handle_t umsg_GPS_data_subscribe(uint32_t prescaler, uint8_t length)
{
    return umsg_subscribe(&msg_GPS_data, prescaler, sizeof(umsg_GPS_data_t), length, 0);
}
umsg_sub_handle_t umsg_GPS_data_subscribe_ch(uint32_t prescaler, uint8_t length, uint8_t channel)
{
    return umsg_subscribe(&msg_GPS_data, prescaler, sizeof(umsg_GPS_data_t), length, channel);
}
void umsg_GPS_data_publish(umsg_GPS_data_t* data)
{
    umsg_publish(&msg_GPS_data, data, 0);
}
void umsg_GPS_data_publish_ch(umsg_GPS_data_t* data, uint8_t channel)
{
    umsg_publish(&msg_GPS_data, data, channel);
}
...
{
    return umsg_peek(&msg_GPS_data, data, sizeof(umsg_GPS_data_t));
}
