...
static umsg_msg_metadata_t msg_LoRa_LoRa_state = {.name = "LoRa_LoRa_state"};
...
{
    return umsg_subscribe(&msg_LoRa_LoRa_state, prescaler, sizeof(umsg_LoRa_LoRa_state_t), length, 0);
}
umsg_sub_handle_t umsg_LoRa_LoRa_state_subscribe_ch(uint32_t prescaler, uint8_t length, uint8_t channel)
{
    return umsg_subscribe(&msg_LoRa_LoRa_state, prescaler, sizeof(umsg_LoRa_LoRa_state_t), length, channel);
}
void umsg_LoRa_LoRa_state_publish(umsg_LoRa_LoRa_state_t* data)
{
    umsg_publish(&msg_LoRa_LoRa_state, data, 0);
}
void umsg_LoRa_LoRa_state_publish_ch(umsg_LoRa_LoRa_state_t* data, uint8_t channel)
{
    umsg_publish(&msg_LoRa_LoRa_state, data, channel);
}
...
{
    return umsg_peek(&msg_LoRa_LoRa_state, data, sizeof(umsg_LoRa_LoRa_state_t));
}
