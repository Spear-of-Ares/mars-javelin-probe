...
static umsg_msg_metadata_t msg_GPS_gps_state = {.name = "GPS_gps_state"};
...
{
    return umsg_subscribe(&msg_GPS_gps_state, prescaler, sizeof(umsg_GPS_gps_state_t), length, 0);
}
umsg_sub_handle_t umsg_GPS_gps_state_subscribe_ch(uint32_t prescaler, uint8_t length, uint8_t channel)
{
    return umsg_subscribe(&msg_GPS_gps_state, prescaler, sizeof(umsg_GPS_gps_state_t), length, channel);
}
void umsg_GPS_gps_state_publish(umsg_GPS_gps_state_t* data)
{
    umsg_publish(&msg_GPS_gps_state, data, 0);
}
void umsg_GPS_gps_state_publish_ch(umsg_GPS_gps_state_t* data, uint8_t channel)
{
    umsg_publish(&msg_GPS_gps_state, data, channel);
}
...
{
    return umsg_peek(&msg_GPS_gps_state, data, sizeof(umsg_GPS_gps_state_t));
}
